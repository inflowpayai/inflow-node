...
A vanilla x402 EVM buyer that recognises and pays an InFlow-generated 402. **No `@inflowpayai/*` imports** — that's the
...
The two `BigInteger`-shape quirks — the leading-`00` sign byte on keys whose top bit is set, and the stripped leading
zeros on small keys — are normalized at the example boundary so the rest of the script sees viem's expected
`0x`-prefixed 32-byte form. The seller needs Base Sepolia USDC at `0x036CbD53842c5426634e7929541eC2318f3dCF7e`.
