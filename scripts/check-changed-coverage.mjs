...
function readLcov(lcovFile, out, partialBranches) {
  ...
  if (!existsSync(absolute)) throw new Error(`Missing coverage report: ${lcovFile}. Run pnpm test first.`);
...
function normalizeSourcePath(packageRoot, sourceFile) {
  return sourceFile.startsWith('/') ? relative(repoRoot, sourceFile) : `${packageRoot}/${sourceFile}`;
}
